...
// FINAL v4: Cold boot เปิด GNSS อัตโนมัติ ไม่ต้องกด RESET + 4G reconnect อัตโนมัติ
...
const unsigned long NETWORK_RETRY_INTERVAL = 15000; // ลองเชื่อมต่อ 4G ใหม่ทุก 15 วินาทีถ้าบูตครั้งแรกยังไม่พร้อม
...
void setupGNSS() {
 ...
 // Cold boot ของ A7670E บางครั้งโมเด็มยังไม่พร้อมรับคำสั่ง GNSS ทันที
 // จึงลองเปิด GNSS ซ้ำหลายครั้ง โดยไม่ต้องให้ผู้ใช้กด RESET
 bool gnssPowerOK = false;

 for (int attempt = 1; attempt <= 4; attempt++) {
  SerialMon.print("GNSS start attempt ");
 ...
 if (gnssPowerOK) {
  SerialMon.println("GNSS started automatically");
 } else {
  SerialMon.println("GNSS start command not confirmed; loop will keep reading/retrying");
 }
}
...
// ส่ง Firebase แบบทนต่อ HTTP session หลุด
bool httpPutJsonReliable(String path, String json, int maxAttempts = 3) {
 for (int attempt = 1; attempt <= maxAttempts; attempt++) {
  SerialMon.print("Firebase attempt ");
  SerialMon.print(attempt);
  SerialMon.print("/");
  SerialMon.println(maxAttempts);

  if (httpPutJson(path, json)) return true;

  SerialMon.println("Retry Firebase with fresh HTTP session...");
 ...
 SerialMon.println("Firebase failed after retries");
 return false;
}
...
bool sendToFirebase() {
 ...
 // ตำแหน่งสด: พยายามส่งซ้ำเมื่อ HTTP session มีปัญหา
 bool latestOK = httpPutJsonReliable(String(FIREBASE_PATH), json, 2);

 unsigned long now = millis();

 // History ครบกำหนดทุก 30 วินาที
 // สำคัญ: อัปเดต lastHistoryUpload เมื่อบันทึกสำเร็จจริงเท่านั้น
 if (latestOK && now - lastHistoryUpload >= HISTORY_INTERVAL) {
  if (latestLocalDate.length() == 10 && latestLocalTime.length() >= 8) {
   ...
   bool historyOK = httpPutJsonReliable(historyPath, json, 3);

   if (historyOK) {
    lastHistoryUpload = now;
    SerialMon.println("HISTORY SAVED OK");
   } else {
    SerialMon.println("HISTORY SAVE FAILED - will retry next upload cycle");
   }
  } else {
...
bool ensureInternet() {
 ...
 SerialMon.println("Checking / reconnecting 4G...");

 if (!modem.isNetworkConnected()) {
  if (!modem.waitForNetwork(30000L)) {
   SerialMon.println("4G network not ready yet");
   internetReady = false;
   return false;
  }
 }
 ...
 if (!modem.isGprsConnected()) {
  if (!modem.gprsConnect(apn, user, pass)) {
   SerialMon.println("Internet not ready yet");
   internetReady = false;
   return false;
  }
 }
 ...
 internetReady = true;
 httpReady = false; // ให้ HTTP setup ใหม่หลัง reconnect
...
void setup() {
 ...
 SerialMon.println("LILYGO A7670E GPS Tracking - AUTO START");
 ...
 // รอและปลุกโมเด็มหลังจ่ายไฟใหม่ โดยไม่ต้องกด RESET
 ...
 if (!modemResponding) {
  SerialMon.println("Modem still waking up; continue and retry in loop");
 }
 ...
 // สำคัญ: เปิด GNSS ก่อนรอเครือข่าย 4G
 // ต่อให้ SIM/เครือข่ายยังไม่พร้อม ดาวเทียมก็เริ่มจับได้ทันที
 setupGNSS();

 // ลองเชื่อมต่อ 4G แต่ห้าม return จนทำให้ GNSS หยุดทำงาน
 if (ensureInternet()) {
  setupHTTP();
 } else {
  SerialMon.println("GNSS is running; 4G will retry automatically");
 }
...
void loop() {
 ...
 // ถ้า 4G ยังไม่พร้อมจาก cold boot ให้ลองใหม่เอง
 if (!internetReady && now - lastNetworkRetry >= NETWORK_RETRY_INTERVAL) {
 ...
 if (now - lastGPSRead >= GPS_INTERVAL) {
  ...
  if (!readGPS()) {
   // ถ้าอ่าน GPS ไม่ได้ต่อเนื่อง อาจเป็นเพราะ GNSS power command ตอน cold boot ไม่ติด
   // ลองสั่งเปิดอีกครั้งเป็นระยะ ไม่ต้องกด RESET
   static unsigned long lastGnssRetry = 0;
   if (now - lastGnssRetry >= 10000) {
    lastGnssRetry = now;
    String pwr = sendAT("AT+CGNSSPWR?", 2000);
    if (pwr.indexOf("+CGNSSPWR: 1") < 0) {
     SerialMon.println("GNSS appears OFF -> restarting GNSS automatically");
     setupGNSS();
    }
   }
  }
 ...
 if (internetReady && now - lastUpload >= UPLOAD_INTERVAL) {
  lastUpload = now;
  sendToFirebase();
 }
}
